...
namespace ego_planner
{
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    static ros::Time global_time_start = ros::Time::now();
    ...
    case SEQUENTIAL_START: // for swarinfom
    {
    ...
    case EXEC_TRAJ:
    {
      ...
      std::cout << "drone_ID " << planner_manager_->pp_.drone_id << " t_cur= " << std::fixed << std::setprecision(6) <<(time_now-global_time_start).toSec() << " pos= " << pos.transpose() << endl;
